...
public:
  ...
  void addTetrahedron(std::array<int, 4> t);
  VectorfieldIsosurface getResultAndReset();
  
private:
  typedef std::vector<glm::vec3>::size_type index_type;
  ...
  index_type getIsopointIndex(const Edge& edge);
  ...
  // internal
  std::map<Edge, index_type> edge_indices;
...
VectorfieldIsosurfaceCalculation::index_type VectorfieldIsosurfaceCalculation::getIsopointIndex(const VectorfieldIsosurfaceCalculation::Edge& edge) {
...
void VectorfieldIsosurfaceCalculation::addTetrahedron(std::array<int, 4> t) {
  int index = 0;
  for (int i = 0; i < 4; i++) {
...
VectorfieldIsosurface VectorfieldIsosurface::calculate(const std::vector<glm::vec3>& positions, const std::vector<glm::vec3>& directions, const std::vector<float>& values, float isovalue, const std::vector<std::array<int, 4>>& tetrahedra) {
