...
{
    // This function is a replacement for the Hamiltonian_Heisenberg member function of the same name, it can be used
...
__device__ bool cu_metropolis_spin_trial(
    ...
    const scalar rng2, const scalar rng3, const scalar cos_cone_angle,
    const scalar temperature ) // aditional parameter kB_T needs to be input
{
    ...
    {
        if( temperature
            < 1e-12 ) // usman: Need to explicitly define the value of temperature rather than just passing the pointer
        {
            // Restore the spin
...
__global__ void cu_metropolis_order(
    const Vector3 * spins_old, Vector3 * spins_new, int * order, unsigned int * counter, Hamiltonian_Device_Ptrs ham,
    const int * phase, const int * n_blocks, const int * block_size_min, int * rest, curandState * states )
{
        ...
        for( int bb = 0; bb < block_size_b; bb++ )
        {
            for( int aa = 0; aa < block_size_a; aa++ )
            {
                for( int ibasis = 0; ibasis < ham.geometry.n_cell_atoms; ibasis++ )
                {
                    ...
                    // int ispin, Vector3 * spins_old, Vector3 * spins_new, Hamiltonian_Device_Ptrs ham, const scalar
                    // rng1, const scalar rng2, const scalar rng3, const scalar cos_cone_angle, scalar temperature,
                    // const scalar kB_T )
    ...
    for( int phase_c = 0; phase_c < phase_c_lim; phase_c++ )
    {
        for( int phase_b = 0; phase_b < phase_b_lim; phase_b++ )
        {
            for( int phase_a = 0; phase_a < phase_a_lim; phase_a++ )
            {
                ...
                cu_parallel_metropolis<<<grid, block>>>(
                    spins_old.data(), spins_new.data(), ham_ptrs, phase.data(), n_blocks.data(), block_size_min.data(),
                    rest.data(), dev_random->state, cos_cone_angle, temperature );
            }
        }
    }

    // cu_metropolis_order<<<grid, block>>>( spins_old.data(), spins_new.data(), order.data(), counter.data(), ham_ptrs,
    // 1, 0, 0, rest[0],rest[1], rest[2], 2, 2, 1);  // cu_metropolis_order<<<grid, block>>>( spins_old.data(),
    // spins_new.data(), order.data(), counter.data(), ham_ptrs, phase_a, phase_b, phase_c, rest[0],rest[1], rest[2],
    // block_size_min[0], block_size_min[1], block_size_min[2]);
