...
namespace Engine
{
    namespace Manifoldmath
    {
        ...
        // This calculates the basis via orthonormalization
        void tangent_basis(const vectorfield & vf, MatrixX & basis)
        {
            ...
            Vector3 v1, v2;
            scalar proj1 = 0, proj2 = 0, proj3 = 0;
            ...
            for(int i=0; i < size; ++i)
            {
                ...
                // Note: our axis is normalized already
                //u1 = axis
                //u2 = v1 = vx - vx*axis/|axis|^2 * axis
                //u3 = v2 = vy - vy*axis/|axis|^2 * axis - vy*v1/|v1|^2 * v1

                // First vector: orthogonalize e1 w.r.t. axis
                v1 = e1 - e1.dot(axis) * axis;
                v1.normalize();

                // Second vector: orthogonalize e2 w.r.t. axis and v1
                v2 = e2 - e2.dot(axis)*axis - e2.dot(v1)*v1;
                v2.normalize();
                
                // Insert into matrix
                basis.block<3,1>(3*i,2*i)   = v1;
                basis.block<3,1>(3*i,2*i+1) = v2;
            }
            
            // // Normalize columns so the transformation does not change vector normalisations
            // // this should already be the case as v1 and v2 are normalized
