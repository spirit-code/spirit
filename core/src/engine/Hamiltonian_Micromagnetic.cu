...
namespace Engine
{
	Hamiltonian_Micromagnetic::Hamiltonian_Micromagnetic(
		...
		anisotropy_magnitude(anisotropy_magnitude), exchange_tensor(exchange_tensor), dmi_tensor(dmi_tensor),
...
/*    void Hamiltonian_Micromagnetic::E_DDI(const vectorfield & spins, scalarfield & Energy)
    ...
    void Hamiltonian_Micromagnetic::Gradient(const vectorfield & spins, vectorfield & gradient)
    {
		...
		{
			energy0 += 0.5 *Ms* gradient[icell].dot(spins[icell]);
		}
        // Anisotropy
        ...
        for (int icell = 0; icell < geometry->n_cells_total; ++icell)
		{
			energy1 += 0.5 *Ms* gradient[icell].dot(spins[icell]);
		}
        // Exchange
        ...
        for (int icell = 0; icell < geometry->n_cells_total; ++icell)
		{
			energy2 += 0.5 *Ms* gradient[icell].dot(spins[icell]);
		}
        // DMI
        ...
        for (int icell = 0; icell < geometry->n_cells_total; ++icell)
		{
			energy3 += 0.5 *Ms* gradient[icell].dot(spins[icell]);
		}
        // DDI
		...
		for (int icell = 0; icell < geometry->n_cells_total; ++icell)
		{
			energy4 += 0.5 *Ms* gradient[icell].dot(spins[icell]);
		}
		printf("Energy ext: %f anisotropy: %f exch: %f dmi: %f ddi: %f total: %f\n", energy0/ geometry->n_cells_total, (energy1-energy0)/ geometry->n_cells_total,(energy2-energy1)/ geometry->n_cells_total,(energy3-energy2)/ geometry->n_cells_total,(energy4-energy3)/ geometry->n_cells_total,energy4/ geometry->n_cells_total);
	...
	__global__ void CU_Gradient_Zeeman1(const int * atom_types, const int n_cell_atoms, const scalar * mu_s, const scalar external_field_magnitude, const Vector3 external_field_normal, Vector3 * gradient, size_t n_cells_total)
	{
			...
			icell += blockDim.x * gridDim.x)
		{
			for (int ibasis = 0; ibasis < n_cell_atoms; ++ibasis)
			{
				int ispin = icell + ibasis;
				if (cu_check_atom_type(atom_types[ispin]))
					gradient[ispin] -=8e5 * external_field_magnitude*external_field_normal;
			}
		}
	}
	...
	__global__ void CU_Gradient_Anisotropy1(const Vector3 * spins, const int * atom_types, const int n_cell_atoms, Vector3 * gradient, size_t n_cells_total, const scalar Ms, const scalar anisotropy_mag)
	{
		Vector3 temp1{ 1,0,0 };
		Vector3 temp2{ 0,1,0 };
		Vector3 temp3{ 0,0,1 };
			...
			icell += blockDim.x * gridDim.x)
		{
			...
			{
				int ispin = icell;
				//gradient[ispin] -= 2.0 * 500000 / Ms * temp1 * temp1.dot(spins[ispin]);
	...
	{
		int size = geometry->n_cells_total;
		CU_Gradient_Anisotropy1 << <(size + 1023) / 1024, 1024 >> > (spins.data(), this->geometry->atom_types.data(), this->geometry->n_cell_atoms, gradient.data(), size, Ms, anisotropy_magnitude);
		CU_CHECK_AND_SYNC();
	}
	...
	__global__ void CU_Gradient_Exchange1(const Vector3 * spins, const int * atom_types, const int * boundary_conditions, const int * n_cells, int n_cell_atoms,
		int n_pairs, const Pair * neigh, Vector3 * gradient, size_t size, bool A_is_nondiagonal, Matrix3 exchange_tensor, const Vector3 cell_lengths, const scalar Ms)
	{
			...
			icell += blockDim.x * gridDim.x)
		{
			...
			for (unsigned int i = 0; i < 3; ++i)
			{
				...
				gradient[ispin][0] -= 2 * exchange_tensor(i, i) / Ms * (spins[ispin_plus][0] - 2 * spins[ispin][0] + spins[ispin_minus][0]) / (cell_lengths[i]) / (cell_lengths[i]);
				gradient[ispin][1] -= 2 * exchange_tensor(i, i) / Ms * (spins[ispin_plus][1] - 2 * spins[ispin][1] + spins[ispin_minus][1]) / (cell_lengths[i]) / (cell_lengths[i]);
				gradient[ispin][2] -= 2 * exchange_tensor(i, i) / Ms * (spins[ispin_plus][2] - 2 * spins[ispin][2] + spins[ispin_minus][2]) / (cell_lengths[i]) / (cell_lengths[i]);
	...
	void Hamiltonian_Micromagnetic::Gradient_Exchange(const vectorfield & spins, vectorfield & gradient)
	{
		...
		CU_Gradient_Exchange1 << <(size + 1023) / 1024, 1024 >> > (spins.data(), this->geometry->atom_types.data(), boundary_conditions.data(), geometry->n_cells.data(), geometry->n_cell_atoms,
			this->neigh.size(), this->neigh.data(), gradient.data(), size, A_is_nondiagonal, exchange_tensor,cell_sizes, Ms);
		CU_CHECK_AND_SYNC();
	}
	...
	__global__ void CU_Spatial_Gradient(const Vector3 * spins, const int * atom_types, const int * boundary_conditions, const int * n_cells, int n_cell_atoms,
		int n_pairs, const Pair * neigh, Matrix3 * spatial_gradient, size_t size, const Vector3 cell_lengths, const scalar Ms)
	{
	...
	void Hamiltonian_Micromagnetic::Spatial_Gradient(const vectorfield & spins)
	{
		...
		CU_Spatial_Gradient << <(size + 1023) / 1024, 1024 >> > (spins.data(), this->geometry->atom_types.data(), boundary_conditions.data(), geometry->n_cells.data(), geometry->n_cell_atoms,
			this->neigh.size(), this->neigh.data(), spatial_gradient.data(), size, cell_sizes, Ms);
		CU_CHECK_AND_SYNC();
	}

	__global__ void CU_Gradient_DMI1(const Vector3 * spins, Vector3 * gradient, const Matrix3 * spatial_gradient, size_t size, const Matrix3 dmi_tensor, const scalar Ms)
	{
		for (auto icell = blockIdx.x * blockDim.x + threadIdx.x;
			icell < size;
			icell += blockDim.x * gridDim.x)
	...
	{
		int size = geometry->n_cells_total;
		CU_Gradient_DMI1 << <(size + 1023) / 1024, 1024 >> > (spins.data(), gradient.data(), spatial_gradient.data(), size, dmi_tensor, Ms);
		CU_CHECK_AND_SYNC();
	}
	...
	__global__ void CU_Write_FFT_Gradients1(const FFT::FFT_real_type * resiFFT, Vector3 * gradient, FFT::StrideContainer spin_stride, int * iteration_bounds, int n_cell_atoms, scalar * mu_s, int sublattice_size, const scalar Ms)
	{
		...
		for (int idx_orig = blockIdx.x * blockDim.x + threadIdx.x; idx_orig < nos; idx_orig += blockDim.x * gridDim.x)
		{
			...
			gradient[idx_orig][0] -= resiFFT[idx_pad]*Ms*1e-7/(sublattice_size);
			gradient[idx_orig][1] -= resiFFT[idx_pad + 1 * spin_stride.comp]*Ms*1e-7/(sublattice_size);
			gradient[idx_orig][2] -= resiFFT[idx_pad + 2 * spin_stride.comp]*Ms*1e-7/(sublattice_size);
		}
	}
