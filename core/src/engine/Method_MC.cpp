...
bool Method_MC::Metropolis_Spin_Trial(
    int ispin, const vectorfield & spins_old, vectorfield & spins_new, const scalar & rng1, const scalar & rng2,
    const scalar & rng3, const scalar & cos_cone_angle )
{
    Matrix3 local_basis;
...
void Method_MC::Block_Decomposition()
{
    ...
    interactions, could look like this: ┌───┬───┬───┬─────┐ │1 1│2 2│1 1│2 2 2│ │   │   │   │     │ │1 1│2 2│1 1│2 2 2│
        ...
        │3 3│4 4│3 3│4 4 4│
        └───┴───┴───┴─────┘
    The decomposition enables us to run the MC algorithm in four serial phases (1 to 4), while each phase is
    parallelized. All boxes belonging to one specific phase, do not interact directly between each other, therefore we
    can perform their MC trial moves in parallel. The number of phases is 2^dim and the number of usable threads is the
    ...
    */
    ...
    Log.Send(
        Utility::Log_Level::Info, Utility::Log_Sender::MC,
        fmt::format(
            "    Block size from interactions = ({}, {}, {})", block_size_min[0], block_size_min[1],
            block_size_min[2] ) );
    ...
    for( int i = 0; i < 3; i++ )
    {
        ...
        { // If the block size is larger than n_cells, we use n_blocks=1
            n_blocks[i]       = 1;
    ...
    Log.Send(
        Utility::Log_Level::Info, Utility::Log_Sender::MC,
        fmt::format(
            "    Actual block size = ({}, {}, {})", block_size_min[0], block_size_min[1], block_size_min[2] ) );
    Log.Send(
        Utility::Log_Level::Info, Utility::Log_Sender::MC,
        fmt::format( "    N_blocks          = ({}, {}, {})", n_blocks[0], n_blocks[1], n_blocks[2] ) );
    Log.Send(
        Utility::Log_Level::Info, Utility::Log_Sender::MC,
        fmt::format( "    Rest              = ({}, {}, {})", rest[0], rest[1], rest[2] ) );
    max_supported_threads
        = std::max( 1, n_blocks[0] / 2 ) * std::max( 1, n_blocks[1] / 2 ) * std::max( 1, n_blocks[2] / 2 );
    Log.Send(
        Utility::Log_Level::Info, Utility::Log_Sender::MC,
        fmt::format( "    Supports up to max. {} threads.", max_supported_threads ) );
}
...
#pragma omp parallel for collapse( 3 ) private( distribution ) reduction( + : n_rejected )
                for( int block_c = phase_c; block_c < n_blocks[2]; block_c += 2 ) // Note the += 2 increment!
                {
                    for( int block_b = phase_b; block_b < n_blocks[1]; block_b += 2 )
                    {
                        ...
                        {
                            int block_size_c
                                = ( block_c == n_blocks[2] - 1 ) ?
                                      block_size_min[2] + rest[2] :
                                      block_size_min[2]; // Account for the remainder of division (n_cells[i] /
                                                         // block_size_min[i]) by increasing the block size at the edges
                            int block_size_b
                                = ( block_b == n_blocks[1] - 1 ) ? block_size_min[1] + rest[1] : block_size_min[1];
                            int block_size_a
                                = ( block_a == n_blocks[0] - 1 ) ? block_size_min[0] + rest[0] : block_size_min[0];
                                    ...
                                    for( int aa = 0; aa < block_size_a; aa++ )
                                    {
                                        for( int ibasis = 0; ibasis < geom->n_cell_atoms; ibasis++ )
                                        {
                                            int a = block_a * block_size_min[0]
                                                    + aa; // We do not have to worry about the remainder of the division
                                                          // here, it is contained in the 'aa'/'bb'/'cc' offset
                                            ...
                                            int ispin = ibasis
                                                        + geom->n_cell_atoms
                                                              * ( a + geom->n_cells[0] * ( b + geom->n_cells[1] * c ) );

                                            // Perform the Metropolis trial step
                                            if( Vectormath::check_atom_type(
                                                    this->systems[0]->geometry->atom_types[ispin] ) )
                                            {
                                                int tid = 0;
#ifdef SPIRIT_USE_OPENMP
                                                ...
                                                if( !Metropolis_Spin_Trial(
                                                        ispin, spins_old, spins_new, rng1, rng2, rng3,
                                                        cos_cone_angle ) )
                                                {
                                                    n_rejected++;
                                                }
