...
namespace Engine
{
	...
	void Hamiltonian::Hessian(const std::vector<double> & spins, std::vector<double> & hessian)
	{
		...
		// Calculate finite difference
		std::vector<double> spins_p(3 * nos, 0);
		std::vector<double> spins_m(3 * nos, 0);

		std::vector<std::vector<double>> grad_p(3 * nos, std::vector<double>(3 * nos));
		std::vector<std::vector<double>> grad_m(3 * nos, std::vector<double>(3 * nos));

		std::vector<double> d(3 * nos);
    ...
    void Hamiltonian::Effective_Field(const std::vector<double> & spins, std::vector<double> & field)
    {
		...
		for (int i = 0; i < 3 * nos; ++i)
		{
			...
			double d = Utility::Manifoldmath::Dist_Geodesic(spins_minus, spins_plus);
