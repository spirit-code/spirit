...
void Hamiltonian_Heisenberg::E_DDI( const vectorfield & spins, scalarfield & Energy )
{
    ...
    __global__ void CU_E_Zeeman(
        const Vector3 * spins, const int * atom_types, const int n_cell_atoms, const scalar * mu_s,
        const scalar external_field_magnitude, const Vector3 external_field_normal, scalar * Energy,
        size_t n_cells_total )
    {
        for( auto icell = blockIdx.x * blockDim.x + threadIdx.x; icell < n_cells_total;
    ...
    void Hamiltonian_Heisenberg::E_Anisotropy( const vectorfield & spins, scalarfield & Energy )
    {
        ...
        CU_E_Anisotropy<<<( size + 1023 ) / 1024, 1024>>>(
            ...
            this->anisotropy_indices.size(), this->anisotropy_indices.data(), this->anisotropy_magnitudes.data(),
            this->anisotropy_normals.data(), Energy.data(), size );
        CU_CHECK_AND_SYNC();
    }
        ...
        const int * anisotropy_indices, const scalar * anisotropy_magnitude, scalar * Energy, size_t n_cells_total )
    {
             ...
             icell += blockDim.x * gridDim.x )
        {
            for( int iani = 0; iani < n_anisotropies; ++iani )
            {
                int ispin = icell * n_cell_atoms + anisotropy_indices[iani];
                if( cu_check_atom_type( atom_types[ispin] ) )
                    Energy[ispin] -= anisotropy_magnitude[iani] / 2
                                     * ( pow( spins[ispin][0], 4.0 ) + pow( spins[ispin][1], 4.0 )
                                         + pow( spins[ispin][2], 4.0 ) );
            }
        }
    }
...
#pragma omp parallel for
    ...
    void Hamiltonian_Heisenberg::E_DDI_Cutoff( const vectorfield & spins, scalarfield & Energy )
    {
        ...
        //                     int idx_i = idx_from_translations(geometry->n_cells, geometry->n_cell_atoms,
        //                     translations); int idx_j = idx_from_translations(geometry->n_cells,
        //                     geometry->n_cell_atoms, translations, ddi_pairs[i_pair].translations); Energy[idx_i] -=
        //                     mult / std::pow(ddi_magnitudes[i_pair], 3.0) *
            ...
            for( int idx2 = 0; idx2 < geometry->nos; idx2++ )
            {
                ...
                for( int a_pb = -img_a; a_pb <= img_a; a_pb++ )
                {
                    for( int b_pb = -img_b; b_pb <= img_b; b_pb++ )
                    {
                        ...
                        {
                            diff_img = diff
                                       + a_pb * geometry->n_cells[0] * geometry->bravais_vectors[0]
    ...
    void Hamiltonian_Heisenberg::Gradient_Anisotropy( const vectorfield & spins, vectorfield & gradient )
    {
        ...
        CU_Gradient_Anisotropy<<<( size + 1023 ) / 1024, 1024>>>(
            spins.data(), this->geometry->atom_types.data(), this->geometry->n_cell_atoms,
            this->anisotropy_indices.size(), this->anisotropy_indices.data(), this->anisotropy_magnitudes.data(),
            this->anisotropy_normals.data(), gradient.data(), size );
        CU_CHECK_AND_SYNC();
    }
        ...
        FFT::StrideContainer spin_stride )
    {
        ...
        for( int ispin = blockIdx.x * blockDim.x + threadIdx.x; ispin < nos; ispin += blockDim.x * gridDim.x )
        {
            ...
            for( int i_b2 = 0; i_b2 < iteration_bounds[0]; i_b2++ )
            {
                ...
                int idx_d
                    = b_inter * dipole_stride.basis + a * dipole_stride.a + b * dipole_stride.b + c * dipole_stride.c;
                    ...
                    for( int alpha = 0; alpha < 3; ++alpha )
                    {
                        for( int beta = 0; beta < 3; ++beta )
                        {
                            ...
                            scalar res = -2.0 * this->anisotropy_magnitudes[iani]
                                         * this->anisotropy_normals[iani][alpha] * this->anisotropy_normals[iani][beta];
    ...
    void Hamiltonian_Heisenberg::FFT_Dipole_Matrices( FFT::FFT_Plan & fft_plan_dipole, int img_a, int img_b, int img_c )
    {
        ...
        // Work around to make bravais vectors and cell_atoms available to GPU as they are currently saves as
        // std::vectors and not fields ...
    ...
    void Hamiltonian_Heisenberg::Prepare_DDI()
    {
        ...
        for( int i = 0; i < 3; i++ )
        {
            ...
            bool perform_zero_padding
                = geometry->n_cells[i] > 1 && ( boundary_conditions[i] == 0 || ddi_pb_zero_padding );
