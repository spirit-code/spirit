        ...
        for( int icell = 0; icell < geometry->n_cells_total; ++icell )
        {
            for( int iani = 0; iani < anisotropy_indices.size(); ++iani )
            {
                ...
                if (check_atom_type(this->geometry->atom_types[ispin]))
                {
                    ...
                    Energy[ispin] -= this->cubic_anisotropy_magnitude[iani]/2 * ( std::pow(spins[ispin][0],4.0) +
                ...
                for (int iani = 0; iani < anisotropy_indices.size(); ++iani)
                {
                    if (anisotropy_indices[iani] == ibasis)
                    {
                        if (check_atom_type(this->geometry->atom_types[ispin]))
                            ...
                            Energy -= this->cubic_anisotropy_magnitude[iani]/2 * ( std::pow(spins[ispin][0],4.0) +
                                                                                          std::pow(spins[ispin][1],4.0) +
                                                                                          std::pow(spins[ispin][2],4.0));
                    }
                }
        ...
        for( int icell = 0; icell < geometry->n_cells_total; ++icell )
        {
            for( int iani = 0; iani < anisotropy_indices.size(); ++iani )
            {
                ...
                if( check_atom_type(this->geometry->atom_types[ispin]) )
                    ...
                    {
                        gradient[ispin][icomp] -= 2.0 * this->cubic_anisotropy_magnitude[iani] * std::pow(spins[ispin][icomp],3.0);
                    }
            }
        }
    }
