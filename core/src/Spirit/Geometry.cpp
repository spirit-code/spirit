...
void Geometry_Set_N_Cells(State * state, int n_cells_i[3]) noexcept
{
    ...
    for (auto& chain : state->collection->chains)
    {
        ...
        for (auto& system : chain->images)
        {
            ...
            // TODO: positions and atom_types should be generated by Geometry (atom types needs some consideration...)
            auto positions = vectorfield(nos);
            Engine::Vectormath::Build_Spins(positions, ge->cell_atoms, ge->translation_vectors, n_cells);
            intfield cell_atom_types(ge->cell_atoms.size(), 0);
