...
void Hamiltonian_Set_Boundary_Conditions(State *state, const bool * periodical, int idx_image, int idx_chain)
{
    try
    {
        ...
        Log( Utility::Log_Level::Info, Utility::Log_Sender::API,
             "Set boundary conditions to " + std::to_string( periodical[0] ) + " " + 
             std::to_string( periodical[1] ) + " " + std::to_string( periodical[2] ), 
             idx_image, idx_chain );
...
void Hamiltonian_Set_Field(State *state, float magnitude, const float * normal, int idx_image, int idx_chain)
{
    try
    {
        ...
        if (image->hamiltonian->Name() == "Heisenberg (Neighbours)")
        {
            ...
            Log( Utility::Log_Level::Info, Utility::Log_Sender::API,
                 "Set external field to " + std::to_string( magnitude ) + ", direction (" + 
                                 ...
                                 int idx_image, int idx_chain)
{
    try
    {
        ...
        if (image->hamiltonian->Name() == "Heisenberg (Neighbours)")
        {
            ...
            Log( Utility::Log_Level::Info, Utility::Log_Sender::API,
                 "Set anisotropy to " + std::to_string( magnitude ) + ", direction (" +
                 std::to_string( normal[0] ) + "," + std::to_string( normal[1] ) + "," +
                 std::to_string( normal[2] ) + ")", idx_image, idx_chain );
        }
        else if (image->hamiltonian->Name() == "Heisenberg (Pairs)")
...
void Hamiltonian_Set_DDI(State *state, float radius, int idx_image, int idx_chain)
{
    try
    {
        ...
        if (image->hamiltonian->Name() == "Heisenberg (Neighbours)")
        {
            ...
            Log( Utility::Log_Level::Info, Utility::Log_Sender::API, "Set ddi radius to " + 
                 std::to_string(radius), idx_image, idx_chain );
        }
        else if (image->hamiltonian->Name() == "Heisenberg (Pairs)")
        {
            auto ham = (Engine::Hamiltonian_Heisenberg_Pairs*)image->hamiltonian.get();
