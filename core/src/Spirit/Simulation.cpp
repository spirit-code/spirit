...
void Simulation_Apply_Eigenmode( State *state, int idx_mode, int idx_image, int idx_chain ) noexcept
{
    try
    {
        // Fetch correct indices and pointers for image and chain
        ...
        {
            Log( Utility::Log_Level::Warning, Utility::Log_Sender::API, 
                "Eigenmodes cannot be applied while a simulation is running on the image" );
                return;
        }
        else
        {
            image->Lock();
            Engine::Apply_Eigenmode( idx_mode, image, idx_image, idx_chain );
            image->Unlock();
        }
    }
    catch( ... )
