...
namespace Data
{
    Parameters_Method_MC::Parameters_Method_MC(std::string output_folder, std::array<bool, 10> output, long int n_iterations, long int n_iterations_log,
            long int max_walltime_sec, std::shared_ptr<Pinning> pinning, int rng_seed, scalar temperature, scalar acceptance_ratio_i) :
        Parameters_Method(output_folder, {output[0], output[1], output[2] , output[3]},
            ...
            acceptance_ratio(acceptance_ratio_i), temperature(temperature), rng_seed(rng_seed), prng(std::mt19937(rng_seed))
    {
    }
}