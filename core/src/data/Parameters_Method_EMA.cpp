...
namespace Data
{
    Parameters_Method_EMA::Parameters_Method_EMA(std::string output_folder, std::string output_file_tag,
        ...
        long int max_walltime_sec, int n_modes, int n_mode_follow, scalar frequency, scalar amplitude, bool snapshot ) :
        Parameters_Method(output_folder, output_file_tag, {output[0], output[1], output[2]}, n_iterations,
            n_iterations_log, max_walltime_sec, 1e-12 ), output_energy_step(output[3]),
        ...
        frequency(frequency), amplitude(amplitude), snapshot(snapshot)
    {
    }
} // namespace Data
