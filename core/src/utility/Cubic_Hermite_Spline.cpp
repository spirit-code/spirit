    ...
    std::size_t n_interpolations )
{
    ...
    std::size_t n_points = p.size() + ( p.size() - 1 ) * n_interpolations;
    ...
    for( std::size_t i = 0; i < p.size() - 1; ++i )
    {
        ...
        for( std::size_t j = 0; j < n_interpolations + 1; ++j )
        {
            ...
            result[0][idx] = static_cast<scalar>( x0 + t * ( x1 - x0 ) );
            result[1][idx]
                = static_cast<scalar>( h00 * p0 + h10 * p1 + h01 * m0 * ( x0 - x1 ) + h11 * m1 * ( x0 - x1 ) );
        }
    }
