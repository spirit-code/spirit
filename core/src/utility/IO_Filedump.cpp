	...
	namespace IO
	{
		...
		*/
		void Strings_to_File(const std::vector<std::string> text, const std::string name, const int no)
		{
			...
			if (myfile.is_open())
			{
				Log.Send(Log_Level::DEBUG, Log_Sender::ALL, "Started writing " + name);
				for (int i = 0; i < no; ++i) {
					myfile << text[i];
				}
				myfile.close();
				Log.Send(Log_Level::DEBUG, Log_Sender::ALL, "Finished writing " + name);
			}
			else
			{
				Log.Send(Log_Level::L_ERROR, Log_Sender::ALL, "Could not open " + name + " to write to file");
			}
		}
