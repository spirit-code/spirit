...
using ::IO::append_to_file;
using ::IO::write_to_file;
    ...
    const std::string & config_file, const std::shared_ptr<Engine::Spin::Hamiltonian> hamiltonian )
{
    ...
    if( const auto * data = hamiltonian->data<Engine::Spin::Interaction::DDI>(); data != nullptr )
    {
        ...
        config += "### DDI force zero padding with periodic boundary conditions";
        config += fmt::format( "ddi_pb_zero_padding         {}", ddi_pb_zero_padding );
...
void State_to_Config( const State & state, const std::string & cfg, const std::string & comment )
{
    // Header
