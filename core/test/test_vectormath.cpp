...
TEST_CASE( "Vectormath operations", "[vectormath]" )
{
    ...
    {
        Vectormath::fill( vf1, { 0, 0, 1 } );
        auto m = Vectormath::Magnetization( vf1, mu_s_field );
    ...
    SECTION( "Rotate" )
    {
        ...
        Vectormath::rotate( v1_in, v1_axis, angle, v1_out );
        ...
        Vectormath::rotate( v2_in, v2_axis, angle2, v2_out );
        for( unsigned int i = 0; i < 3; i++ )
    ...
    SECTION( "Fill" )
    {
        ...
        Vectormath::fill( sf, stest );
        Vectormath::fill( vf1, vtest );
    ...
    SECTION( "Scale" )
    {
        ...
        Vectormath::scale( sf, stest );
        Vectormath::scale( vf1, stest );
        for( int i = 0; i < N_check; ++i )
        {
            REQUIRE( sf[i] == stest );
            REQUIRE( vf1[i] == vtest );
    ...
    SECTION( "Sum, Mean and Divide" )
    {
        ...
        scalar stest1  = Vectormath::sum( sf );
        Vector3 vtest1 = Vectormath::sum( vf1 );
        Vector3 vtest2 = Vectormath::sum( vf2 );
        REQUIRE( stest1 == sN );
        ...
        scalar stest2  = Vectormath::mean( sf );
        Vector3 vtest3 = Vectormath::mean( vf1 );
        Vector3 vtest4 = Vectormath::mean( vf2 );
        REQUIRE( stest2 == 1 );
        ...
        Vectormath::divide( numerator, denominator, result );
        for( int i = 0; i < N_check; i++ )
        {
    ...
    {
        scalar mc = Vectormath::max_abs_component( vf1 );
        REQUIRE( mc == 1 );

        Vector3 vtest1 = vf1[0].normalized();
        Vector3 vtest2 = vf2[0].normalized();
        Vectormath::normalize_vectors( vf1 );
        Vectormath::normalize_vectors( vf2 );
        for( int i = 0; i < N_check; ++i )
        {
    ...
    SECTION( "MAX Abs component" )
    {
        ...
        Vectormath::normalize_vectors( vf1 );
        Vectormath::normalize_vectors( vf2 );
        scalar vfmc1 = Vectormath::max_abs_component( vf1 );
        scalar vfmc2 = Vectormath::max_abs_component( vf2 );
    ...
    SECTION( "MAX norm" )
    {
        ...
        scalar max_norm  = Vectormath::max_norm( vf1 );
    ...
    SECTION( "Dot and Cross Product" )
    {
        ...
        Vectormath::dot( vf1, vf2, dots );
        REQUIRE_THAT( dots[0], WithinAbs( 1, 1e-12 ) );
        REQUIRE_THAT( Vectormath::dot( vf1, vf2 ), WithinAbs( N, 1e-12 ) );

        // Scalarfields dot Product
        scalarfield sf1( N, 2 );
        scalarfield sf2( N, -0.5 );
        scalarfield result( N, 0 );
        Vectormath::dot( sf1, sf2, result );
        for( int i = 0; i < N; i++ )
            REQUIRE( result[i] == -1 );
        ...
        Vectormath::cross( vf1, vf2, crosses );
    ...
    SECTION( "c*a" )
    {
        ...
        Vectormath::add_c_a( 2, vtest1, vf2 );
        for( int i = 0; i < N_check; ++i )
            REQUIRE( vf2[i] == vtest2 );

        // out[i] += c*a[i]
        Vector3 vtest3{ 0.0, -2.0, -2.0 };
        Vectormath::add_c_a( -1, vf2, vf1 );
        for( int i = 0; i < N_check; ++i )
            REQUIRE( vf1[i] == vtest3 );

        // out[i] = c*a
        Vectormath::set_c_a( 3, vtest1, vf1 ); // vf1 is now { 3, 3, 3 }
        for( int i = 0; i < N_check; ++i )
            REQUIRE( vf1[i] == 3 * vtest1 );

        // out[i] = c*a[i]
        Vectormath::set_c_a( 3, vf1, vf2 ); // vf2 is now { 9, 9, 9 }
        for( int i = 0; i < N_check; ++i )
        ...
        scalarfield sf( N, -1 );
        Vectormath::add_c_a( sf, vf2, vf1 ); // vf1 is now { -6, -6, -6 }
        for( int i = 0; i < N_check; i++ )
            REQUIRE( vf1[i] == vtest4 );

        // out[i] = c[i]*a[i]
        Vector3 vtest5{ 6, 6, 6 };
        Vectormath::set_c_a( sf, vf1, vf2 ); // vf2 is now { 6, 6, 6 }
        for( int i = 0; i < N_check; i++ )
    ...
    SECTION( "c*v1.dot(v2)" )
    {
        ...
        Vectormath::add_c_dot( -3, vtest1, vf1, sf );
        for( int i = 0; i < N_check; ++i )
            REQUIRE( sf[i] == 13 );

        // out[i] += c * a[i]*b[i]
        Vectormath::add_c_dot( -2, vf1, vf2, sf );
        for( int i = 0; i < N_check; ++i )
            REQUIRE( sf[i] == 11 );

        // out[i] = c * a*b[i]
        Vectormath::set_c_dot( 3, vtest1, vf1, sf );
        for( int i = 0; i < N_check; ++i )
            REQUIRE( sf[i] == -12 );

        // out[i] = c * a[i]*b[i]
        Vectormath::set_c_dot( 2, vf1, vf2, sf );
        for( int i = 0; i < N_check; ++i )
            REQUIRE( sf[i] == 2 );
    }
    ...
    SECTION( "c*v1.cross(v2)" )
    {
        ...
        Vectormath::add_c_cross( 4, vf2[0], vf1, vftest );
        for( int i = 0; i < N_check; ++i )
            REQUIRE( vftest[i] == vtest1 );

        // out[i] += c * a[i] x b[i]
        Vector3 vtest2{ 1.0, 1.0, 1.0 };
        Vectormath::add_c_cross( 4, vf1, vf2, vftest );
        for( int i = 0; i < N_check; ++i )
            REQUIRE( vftest[i] == vtest2 );

        // out[i] = c * a x b[i]
        Vector3 vtest3{ 0.0, -6.0, 6.0 };
        Vectormath::set_c_cross( 3, vf1[0], vf2, vftest );
        for( int i = 0; i < N_check; ++i )
            REQUIRE( vftest[i] == vtest3 );

        // out[i] = c * a[i] x b[i]
        Vectormath::set_c_cross( 3, vf1, vf2, vftest );
        for( int i = 0; i < N_check; ++i )
            REQUIRE( vftest[i] == vtest3 );
    }
    ...
    SECTION( "jacobian" )
    {
        ...
        Spirit::Data::Basis_Cell_Composition cell_composition;
        Spirit::Data::Pinning pinning;
        Spirit::Data::Defects defects;

        auto test_geometry = Spirit::Data::Geometry(
        ...
        Vectormath::jacobian( vftest, test_geometry, boundary_conditions, jacobians );
