...
{
    // If the argument of the log gets large, it is zeroed out by the prefactor in f or g. Therefore we just set it to
    // zero here, to avoid division by zero.
...
scalar f( const scalar & x, const scalar & y, const scalar & z )
{
    ...
    return (
        ...
        + ( z / 2.0 ) * ( y - x ) * ( y + x ) * kappa( z, x, y, R ) - x * y * z * delta( y, z, x, R )
...
scalar g( const scalar & x, const scalar & y, const scalar & z )
{
    scalar R = std::sqrt( x * x + y * y + z * z );
    return (
        ( x * y * z ) * kappa( z, x, y, R ) + ( y / 6.0 ) * ( 3 * z * z - y * y ) * kappa( x, y, z, R )
        + ( x / 6.0 ) * ( 3 * z * z - x * x ) * kappa( y, x, z, R ) - ( z * z * z / 6.0 ) * delta( x, y, z, R )
        - ( z * y * y / 2.0 ) * delta( x, z, y, R ) - ( z * x * x / 2.0 ) * delta( y, z, x, R ) - x * y * R / 3.0 );
}
...
template<typename scalar>
scalar
Nxx( const scalar & X, const scalar & Y, const scalar & Z, const scalar & dx, const scalar & dy, const scalar & dz,
     scalar & abs_error )
{
    scalar cur_max = 0;
    scalar res     = 0;
    ...
    // The main sources of error are temporary values with large magnitudes, while the final result of the sum is small
    // in magnitude. Therefore, we approximate the absolute error of the sum as the abosolute error of the largest summand.
...
template<typename scalar>
scalar
Nxy( const scalar & X, const scalar & Y, const scalar & Z, const scalar & dx, const scalar & dy, const scalar & dz,
     scalar & abs_error )
{
    scalar cur_max = 0;
    scalar res     = 0;
