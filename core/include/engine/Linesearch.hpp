...
public:
    RotationLinesearch(int nos) : nos(nos), axes_temp(vectorfield(nos))
    { }
    ...
    void rotate( vectorfield & v, const vectorfield & axes, scalar alpha = 1.0 )
    {
        Backend::par::apply( v.size(), [v = v.data(), a = axes.data(), alpha] SPIRIT_LAMBDA( int idx ) 
        {
            ...
            {
                const scalar angle = alpha * axis.norm();
                const Matrix3 rotation_matrix = Eigen::AngleAxis<scalar>(angle, axis.normalized()).toRotationMatrix().eval();
                const Vector3 vt = rotation_matrix * v[idx];
                v[idx] = vt;// rotation_matrix;//(rotation_matrix * v[idx]).eval();
            }
        } );
    }
    ...
    void compute_axes(const vectorfield & v, const vectorfield & directions, vectorfield & axes)
    {
        ...
        {
            if ( std::abs(v[idx].dot( d[idx].normalized() )) >= v[idx].norm() * (1 - std::numeric_limits<scalar>::epsilon() ) ) // Angle can become nan for collinear spins
                a[idx] = Vector3::Zero();
            else
                a[idx] = v[idx].normalized().cross(d[idx]);

            // printf("%f\n-\n", v_in[idx].dot(d[idx].normalized()));
...
private:
        ...
        std::vector<std::shared_ptr<vectorfield>> & configurations, const std::vector<vectorfield> & directions )
    {
        ...
        for( int img = 0; img < noi; img++ )
        {
            ...
            b[img] = Vectormath::dot( directions[img], gradient_throwaway ); // b is the projection of the gradient along the tangent
            ...
            // To make comparisons between gradient and direction meaningful, we rotate the gradient back into the tangent frame of the start and then project to its tangent plane
    ...
    bool check_energy( scalar alpha, int img )
    {
        ...
        scalar delta_e_expected = b[img] * alpha + a[img] * alpha * alpha;

        // Compute error of expected energy
        scalar error_delta_e_expected
            = std::numeric_limits<scalar>::epsilon()
              * std::sqrt( std::pow( b[img] * alpha, 2 ) + std::pow( a[img] * alpha * alpha, 2 ) );

        // Compute the real energy difference
        scalar delta_e = energy_end[img] - energy_start[img];
        scalar error_delta_e
            = std::numeric_limits<scalar>::epsilon()
        ...
        bool linesearch_applicable = error_ratio < tolerance * 1e-1;
        scalar criterion = std::abs( std::abs( delta_e_expected / delta_e ) - 1 );
