...
//         Backend::par::apply( v.size(), [a = axes.data(), v = v.data(), d = directions.data()] SPIRIT_LAMBDA( int
//         idx ) {
//             if( std::abs( v[idx].dot( d[idx].normalized() ) )
//                 >= v[idx].norm()
//                        * ( 1 - std::numeric_limits<scalar>::epsilon() ) ) // Angle can become nan for collinear
//                        spins
//                 a[idx] = Vector3::Zero();
//             else
...
//     // Check the energy predictions assuming the current energies are stored in energy_end and we are at step
//     length alpha bool check_energy( scalar alpha, int img )
//     {
...
//                 {
//                     alpha /= 2.0;
//                     // propagate( *configurations[img], directions[img], -alpha ); //Note: negative sign is for
//                     backtracking rotate( *configurations[img], axes_temp, -alpha );
