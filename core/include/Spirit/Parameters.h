...
DLLEXPORT void Parameters_Set_GNEB_N_Iterations(State *state, int n_iterations, int idx_chain=-1);
DLLEXPORT void Parameters_Set_GNEB_N_Iterations_Log(State *state, int n_iterations_log, int idx_chain=-1);
...
// Get MC
DLLEXPORT void Parameters_Get_MC_Temperature(State *state, float * T, int idx_image = -1, int idx_chain = -1);
// Get GNEB
DLLEXPORT void Parameters_Get_GNEB_Spring_Constant(State *state, float * spring_constant, int idx_image=-1, int idx_chain=-1);
DLLEXPORT void Parameters_Get_GNEB_Climbing_Falling(State *state, int * image_type, int idx_image=-1, int idx_chain=-1);
DLLEXPORT int Parameters_Get_GNEB_N_Iterations(State *state, int idx_chain=-1);
DLLEXPORT int Parameters_Get_GNEB_N_Iterations_Log(State *state, int idx_chain=-1);
