...
void Append_Image_Energy(
    const Data::Spin_System & s, int iteration, const std::string & filename, bool normalize_by_nos = true,
...
void Write_Chain_Energies(
    const Data::Spin_System_Chain & c, int iteration, const std::string & filename, bool normalize_by_nos = true,
    bool readability_toggle = true );

// Saves the Energies interpolated by the GNEB method
void Write_Chain_Energies_Interpolated(
    const Data::Spin_System_Chain & c, const std::string & filename, bool normalize_by_nos = true,
    bool readability_toggle = true );
