	...
	namespace IO
	{
		...
		public:
			...
			template <typename T> void Read_Single(T & var, const std::string name) {
				...
				else Log.Send(Utility::Log_Level::L_ERROR, Utility::Log_Sender::IO, "Keyword '" + name + "' not found. Using Default: " + stringify(var));
			};
			...
			template <typename T> void Read_3Vector(T & var, const std::string name) {
				...
				else Log.Send(Utility::Log_Level::L_ERROR, Utility::Log_Sender::IO, "Keyword '" + name + "' not found. Using Default: {" + stringify(var[0]) + ", " + stringify(var[1]) + ", " + stringify(var[2]) + "}");
			};
