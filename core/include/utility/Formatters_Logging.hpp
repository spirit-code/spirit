...
struct fmt::formatter<Utility::LogEntry> : formatter<string_view>
{
    ...
    auto format( Utility::LogEntry entry, FmtContext & ctx ) const
    {
        ...
        auto out = fmt::format_to(
            ctx.out(), fmt::runtime( format_str ), std::chrono::time_point_cast<std::chrono::seconds>( entry.time ),
            entry.level, entry.sender, index_to_string( entry.idx_image ), entry.message_lines[0] );
