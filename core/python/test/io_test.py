...
class Image_IO(TestParameters):
    ...
    def test_read(self):
        ...
        io.image_write(
            self.p_state, io_image_test, io.FILEFORMAT_OVF_TEXT, "python io test"
        )
        io.image_read(self.p_state, io_image_test)
        spins = system.get_spin_directions(self.p_state)
        for i in range(nos):
            self.assertAlmostEqual(spins[i][0], 0.0)
            self.assertAlmostEqual(spins[i][1], 0.0)
            self.assertAlmostEqual(spins[i][2], 1.0)
...
class Chain_IO(TestParameters):
    def test_chain_write(self):
        ...
        configuration.plus_z(
            self.p_state,
        )
        # write and append chain
        io.chain_write(
            self.p_state, io_chain_test, io.FILEFORMAT_OVF_TEXT, "python io chain"
