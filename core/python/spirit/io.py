...
def image_read(
    p_state, filename, idx_image_infile=0, idx_image_inchain=-1, idx_chain=-1
):
...
def chain_read(
    p_state, filename, starting_image=0, ending_image=-1, insert_idx=-1, idx_chain=-1
):
    ...
    _Chain_Read(
        ctypes.c_void_p(p_state),
        ctypes.c_char_p(filename.encode("utf-8")),
        ctypes.c_int(starting_image),
        ctypes.c_int(ending_image),
        ctypes.c_int(insert_idx),
        ctypes.c_int(idx_chain),
    )


_Chain_Write = _spirit.IO_Chain_Write
_Chain_Write.argtypes = [
    ctypes.c_void_p,
    ctypes.c_char_p,
    ctypes.c_int,
    ctypes.c_char_p,
...
def chain_write(
    p_state, filename, fileformat=FILEFORMAT_OVF_TEXT, comment="", idx_chain=-1
):
...
_Image_Write_Energy_per_Spin.argtypes = [
    ctypes.c_void_p,
    ctypes.c_char_p,
    ctypes.c_int,
    ctypes.c_int,
...
def image_write_energy_per_spin(
    p_state, filename, fileformat=FILEFORMAT_OVF_TEXT, idx_image=-1, idx_chain=-1
):
