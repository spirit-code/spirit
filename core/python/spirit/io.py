...
def Chain_Read(p_state, filename, idx_chain=-1):
    spiritlib.WrapFunction(_Chain_Read, [p_state, ctypes.c_char_p(filename.encode('utf-8')), idx_chain])
...
_Chain_Write.argtypes    = [ctypes.c_void_p, ctypes.c_char_p, ctypes.c_int]
_Chain_Write.restype     = None
def Chain_Write(p_state, filename, idx_chain=-1):
    spiritlib.WrapFunction(_Chain_Write, [p_state, ctypes.c_char_p(filename.encode('utf-8')), idx_chain])
