...
_LLG_Set_Output_Folder.argtypes = [
    ctypes.c_void_p,
    ctypes.c_char_p,
    ctypes.c_int,
    ctypes.c_int,
...
_LLG_Set_Output_General.argtypes = [
    ctypes.c_void_p,
    ctypes.c_bool,
    ctypes.c_bool,
    ctypes.c_bool,
...
_LLG_Set_Output_Energy.argtypes = [
    ctypes.c_void_p,
    ctypes.c_bool,
    ctypes.c_bool,
    ctypes.c_bool,
...
_LLG_Set_Output_Configuration.argtypes = [
    ctypes.c_void_p,
    ctypes.c_bool,
    ctypes.c_bool,
    ctypes.c_int,
...
_LLG_Set_N_Iterations.argtypes = [
    ctypes.c_void_p,
    ctypes.c_int,
    ctypes.c_int,
    ctypes.c_int,
...
_LLG_Set_Direct_Minimization.argtypes = [
    ctypes.c_void_p,
    ctypes.c_bool,
    ctypes.c_int,
    ctypes.c_int,
...
def set_direct_minimization(p_state, use_minimization, idx_image=-1, idx_chain=-1):
    ...
    """
    _LLG_Set_Direct_Minimization(
        ctypes.c_void_p(p_state),
        ctypes.c_bool(use_minimization),
        ctypes.c_int(idx_image),
        ctypes.c_int(idx_chain),
    )
...
def set_temperature(
    p_state,
    temperature,
    gradient_inclination=0,
    gradient_direction=[1.0, 0.0, 0.0],
    idx_image=-1,
    idx_chain=-1,
):
...
_LLG_Get_N_Iterations.argtypes = [
    ctypes.c_void_p,
...
def get_non_adiabatic_damping(p_state, idx_image=-1, idx_chain=-1):
    ...
    return float(
        _LLG_Get_Non_Adiabatic_Damping(
            ctypes.c_void_p(p_state), ctypes.c_int(idx_image), ctypes.c_int(idx_chain)
        )
    )
