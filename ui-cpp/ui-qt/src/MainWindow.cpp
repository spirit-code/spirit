...
void MainWindow::keyBindings()
{
    QMessageBox::about(
        ...
        QString::fromLatin1(
            "The <b>Key Bindings</b> are as follows:<br>"
            "<br>"
            "<i>UI controls</i><br>"
            " - <b>F1</b>:      Show this<br>"
...
void MainWindow::load_Configuration()
{
    ...
    auto fileName = QFileDialog::getOpenFileName( this, tr( "Open Config" ), "./input", tr( "Config (*.toml *.cfg)" ) );
    if( !fileName.isEmpty() )
    {
        auto file = string_q2std( fileName );
...
void MainWindow::save_Configuration()
{
    int idx_img = System_Get_Index( state.get() );
    // Read Spin System from cfg
    auto fileName = QFileDialog::getSaveFileName( this, tr( "Save Config" ), "./input", tr( "Config (*.toml *.cfg)" ) );
    if( !fileName.isEmpty() )
    {
        auto file = string_q2std( fileName );
