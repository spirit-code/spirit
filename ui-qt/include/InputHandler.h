#ifndef INPUT_HANDLER_H
#define INPUT_HANDLER_H

#include <Qt>
#include <QPoint>

class InputHandler
{
public:

  // Possible key states
  enum InputState
  {
    InputInvalid,
    InputRegistered,
    InputUnregistered,
    InputTriggered,
    InputPressed,
    InputReleased
  };

  // State checking
  static InputState keyState(Qt::Key key);
  static bool keyTriggered(Qt::Key key);
  static bool keyPressed(Qt::Key key);
  static bool keyReleased(Qt::Key key);
  static InputState buttonState(Qt::MouseButton button);
  static bool buttonTriggered(Qt::MouseButton button);
  static bool buttonPressed(Qt::MouseButton button);
  static bool buttonReleased(Qt::MouseButton button);
  static bool mouseWheelTurned();
  static QPoint mousePosition();
  static QPoint mouseDelta();
  static int mouseWheelDelta();

private:

  // State updating
  static void update();
  static void registerKeyPress(int key);
  static void registerKeyRelease(int key);
  static void registerMousePress(Qt::MouseButton button);
  static void registerMouseRelease(Qt::MouseButton button);
  static void registerMouseWheel(int wheelDelta);
  static void reset();
  friend class Spin_Widget;
};

inline bool InputHandler::keyTriggered(Qt::Key key)
{
  return keyState(key) == InputTriggered;
}

inline bool InputHandler::keyPressed(Qt::Key key)
{
  return keyState(key) == InputPressed;
}

inline bool InputHandler::keyReleased(Qt::Key key)
{
  return keyState(key) == InputReleased;
}

inline bool InputHandler::buttonTriggered(Qt::MouseButton button)
{
  return buttonState(button) == InputTriggered;
}

inline bool InputHandler::buttonPressed(Qt::MouseButton button)
{
  return buttonState(button) == InputPressed;
}

inline bool InputHandler::buttonReleased(Qt::MouseButton button)
{
  return buttonState(button) == InputReleased;
}

#endif // QINPUT_H
