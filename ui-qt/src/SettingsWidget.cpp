...
{
	Log(Utility::Log_Level::DEBUG, Utility::Log_Sender::UI, "button Random");
...
{
	Log(Utility::Log_Level::DEBUG, Utility::Log_Sender::UI, "button createSkyrmion");
...
{
	Log(Utility::Log_Level::DEBUG, Utility::Log_Sender::UI, "button createSpinSpiral");
...
{
	Log(Utility::Log_Level::DEBUG, Utility::Log_Sender::UI, "button DomainWall");
...
void SettingsWidget::homogeneousTransitionPressed()
{
	...
	{
		Log(Utility::Log_Level::L_ERROR, Utility::Log_Sender::UI, "First index for homogeneous transition is invalid! setting to 1...");
	...
	if (idx_1 == idx_2)
	{
		Log(Utility::Log_Level::L_ERROR, Utility::Log_Sender::UI, "Indices are equal in homogeneous transition! Aborting...");
		return;
	}
	if (idx_2 < idx_1)
	{
		Log(Utility::Log_Level::L_ERROR, Utility::Log_Sender::UI, "Index 2 is smaller than index 1 in homogeneous transition! Aborting...");
		return;
	}
	...
	auto apply = [this](std::shared_ptr<Data::Spin_System> s) -> void
	{
		...
		try {
			...
			if (ex == Utility::Exception::Division_by_zero) {
				...
				Log(Utility::Log_Level::WARNING, Utility::Log_Sender::UI, "B_vec = {0,0,0} replaced by {0,0,1}");
		...
		try {
			...
			if (ex == Utility::Exception::Division_by_zero) {
				...
				Log(Utility::Log_Level::WARNING, Utility::Log_Sender::UI, "Aniso_vec = {0,0,0} replaced by {0,0,1}");
		...
		try {
			...
			if (ex == Utility::Exception::Division_by_zero) {
				...
				Log(Utility::Log_Level::WARNING, Utility::Log_Sender::UI, "s_c_vec = {0,0,0} replaced by {0,0,1}");
	...
	auto apply = [this](std::shared_ptr<Data::Spin_System> s) -> void
	{
		...
		try {
			...
			if (ex == Utility::Exception::Division_by_zero) {
				...
				temp[1] = 0.0;
				temp[2] = 1.0;
				Log(Utility::Log_Level::WARNING, Utility::Log_Sender::UI, "ani_vec = {0,0,0} replaced by {0,0,1}");
